...
- 🚀 **高性能查询** - 国内域名平均延迟 7.6ms，缓存命中 < 1ms
- 🌐 **智能分流** - 基于 GeoSite/GeoIP 自动识别国内/国外域名，分别转发
...
| 国内 DNS | `223.5.5.5:53`, `119.29.29.29:53` |
| 国外 DNS | `8.8.8.8:853` (DoT) |
| 缓存大小 | 4096 条 |
| 懒缓存 | 启用 |
| 广告过滤 | 启用 |
| 泄漏保护 | 启用 (宽松模式) |
...
DNS 请求
  │
  ├── 自定义规则匹配 ──→ 拦截 / 重定向 / 国内DNS / 国外DNS
  │
  ├── 广告过滤匹配 ──→ 拦截 (返回空响应)
  │
  ├── GeoSite 匹配 ──→ 国内DNS (cn) / 国外DNS (geolocation-!cn)
  │
  ├── 泄漏保护 ──→ 根据模式处理
  │
  └── 默认策略 ──→ 国内DNS / 国外DNS
