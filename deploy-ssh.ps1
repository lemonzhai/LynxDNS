...
function Remote-Exec($cmd) {
    ...
    $sshCommand = "sshpass -p '$Password' ssh -o StrictHostKeyChecking=no $User@$RouterIP '$cmd'"
    Invoke-Expression $sshCommand 2>&1
}

function Remote-Copy($src, $dst) {
    # Use PowerShell SCP with password authentication
    $scpCommand = "sshpass -p '$Password' scp -o StrictHostKeyChecking=no $src $User@$RouterIP:$dst"
